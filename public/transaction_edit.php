...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $repo->updateTransaction($userId,$id,$accountId,$date,$amount,$desc,$categoryId,$notes);
  ...
  <form method="post" class="card p-3 shadow-sm">
    ...
    <div class="row">
      ...
      <div class="col-md-4 mb-3">
        ...
        <input type="text" name="amount" value="<?= App\Util::h((string)$tx['amount']) ?>" class="form-control" required>
      </div>
      <div class="col-md-4 mb-3">
    ...
    <div class="mb-3">
      ...
      <select name="category_id" class="form-select">
        ...
        <?php foreach($categories as $cat): ?>
          <option value="<?= (int)$cat['id'] ?>" <?= sel($cat['id'], $tx['category_id']) ?>>
            <?= App\Util::h($cat['name']) ?>
          </option>
        <?php endforeach; ?>
      </select>
